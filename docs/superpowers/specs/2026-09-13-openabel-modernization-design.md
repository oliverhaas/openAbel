...
| Code scope | Cython 3 migration, the allocation bug, and the four side-path bugs, each with a regression test. No API change: parameter and function names stay camelCase (physics/maths code, not web code); ruff's naming rules are switched off for this repo. |
...
├── tests/  analytic.py  test_input.py  test_methods.py  test_regressions.py  test_package.py
├── docs/   index.md  transform-types.md  transform-methods.md  remarks.md  examples/  reference/  superpowers/
...
`[tool.cibuildwheel]`: `build = ["cp312-*", "cp313-*", "cp314-*", "cp314t-*"]` (cibuildwheel 4 builds
free-threaded targets without an enable flag), `build-frontend = "build[uv]"`, `build-verbosity = 1`,
...
on the `.py` files (ruff never touches `.pyx`/`.pxd`).
...
1. `abel/base.pyx:59`: the `else` branch of the method dispatch raises inside a nested `with gil:` while
   the GIL is already held; Cython 3 rejects that. Drop the inner `with gil:`, keep the `raise`.
...
### 2.3 Side-path bugs (each with a regression test, section 3)
...
- `tests/analytic.py` (helper, not collected): the Gaussian `exp(-x^2)` test pair for every transform type
  ...
  formulas of the 1152-combination baseline sweep.
- `tests/test_methods.py`: behaviour preservation. `nData = 200`, `xMax = 3.5`, error metric
  ...
  above the observed value and far above any BLAS/platform noise (~1e-15). Two cells deviate: the `o10`
  cells sit 10-100x above the rule (the measured errors are 1e-15 to 7e-13, within reach of BLAS noise),
  and `fb=2, method=1` is `2e-1` (three times the measured `6.4e-2`; the rule would give 1):

  | fb | o1 | o2 | o3 | o5 | o10 | m0 | m1 |
  |---|---|---|---|---|---|---|---|
  | -1 | 1e-2 | 1e-4 | 1e-6 | 1e-9 | 1e-13 | 1e-2 | 1e-2 |
  | 1 | 1e-1 | 1e-1 | 1e-4 | 1e-6 | 1e-11 | 1e-1 (expected; see below) | 1e-1 |
  ...
  `2.2e-4`.) Two combinations never ran before: `fb=1, method=0` (bug a) and `fb=-2, shift=0.5` with
  `method=2` and `method=3` (bug b). Their tolerances are set when the fixes land, by the same rule
  (measured error times 5, rounded up to a power of ten), with the measured value recorded in a comment;
  for `fb=-2, shift=0.5` the table row is kept if the measured errors already satisfy it.
- `tests/test_regressions.py`, one test per bug: (a) `Abel(200, 1, 0.0, step, method=0)` constructs and its
  output is finite and within tolerance; (b) `fb=-2, shift=0.5` with `method=2` and `method=3` construct,
  agree with each other to `1e-8` and meet the `fb=-2` tolerances; (c) `Abel(200, fb, 0.25, 0.01,
  method=3)` for `fb in (-1, 1, -2)` raises `NotImplementedError` (no crash); (d) `Abel(200, -2, 0.0,
  step, method=1)` raises `NotImplementedError` and `Abel(200, -1, 0.0, step, method=1).execute(...)`
  still works afterwards.
- `tests/test_input.py`: `method in (-1, 5)` raise `NotImplementedError` (the old file defined the same
  test name twice, so `-1` was never tested); `order=0` raises `ValueError`; unsupported `shift` with
  `method=2` raises `NotImplementedError`.
...
`ignore = ["COM812", "D", "E501", "EM", "N", "TRY003"]` (`N`: camelCase API and module name are
deliberate), `extend-exclude = ["add"]`. Per-file: `tests/**` = `["ANN", "INP001", "PLR2004", "S101",
"PT011"]`; `examples/**` = `["ANN", "ERA001", "INP001", "PLR2004", "T201"]`. ruff sees only `.py` files
(`__init__.py`, `coeffs.py`, `setup.py`, tests, examples); never pass `.pyx` paths to it.

ty: `uv run ty check src tests`; the single compiled-import site carries an inline ignore (section 1).
...
`uv run ty check src tests` as the pre-push hook and `default_language_version: python3.12`.

## 5. CI

`.github/workflows/ci.yml`, on push to `main` and pull requests to `main`:

- `lint` (ubuntu-latest, `UV_PYTHON: "3.14"`): `uv sync --group dev --group docs`, `uv lock --check`,
  `ruff check`, `ruff format --check`, `ty check src tests`, `mkdocs build --strict`.
- `test`, `fail-fast: false`, matrix `os: ubuntu-latest` x `python: ["3.12", "3.13", "3.14", "3.14t"]`
  plus `os: macos-latest, python: "3.14"`; each leg `uv python install`, `uv sync --group dev` (this
  compiles the extensions) and `uv run pytest`, with `UV_PYTHON` set to the matrix version so the
  free-threaded leg cannot pick up the runner's system interpreter.
- Action pins: `actions/checkout@v6`, `astral-sh/setup-uv@v7` (dependabot bumps them).
...
- `publish.yml.disabled`: the django-filthyfields shape. On `v*` tags or `workflow_dispatch(version)`: a
  `test` job (ruff, ty, pytest), `build_wheels` on ubuntu-latest and macos-latest with `pypa/cibuildwheel`
  (consumes `[tool.cibuildwheel]`), `build_sdist` with `uv build --sdist`, `publish` with
  `actions/download-artifact` (`merge-multiple: true`) and `pypa/gh-action-pypi-publish@release/v1` under
  `environment: pypi` with `id-token: write` (Trusted Publishing, no token secret).
- `tag.yml.disabled`: after a green CI run on `main`, read `version` from `pyproject.toml`, skip if the
  version is on PyPI or the tag exists, otherwise tag `v<version>` and `gh workflow run publish.yml`.
...
GitHub Pages from the `gh-pages` branch; rename the three files; delete the Read the Docs project or leave
a redirect; set `[project.urls] Documentation`.
...
snippet, replacing `literalinclude`), `reference/api.md` (the `Abel.__init__` and `Abel.execute`
parameter documentation from `wrap.pyx`'s docstrings, hand-written because griffe cannot parse `.pyx`),
`reference/changelog.md` (`0.7.0` entry listing sections 1-2; earlier versions: "no changelog was kept").
...
the method overview, the citation request and the GPL notice.
...
3. Implementation order in the plan: packaging skeleton and src move (build must pass before anything
   else), Cython 3 and allocation fixes with the ported tests green on 3.12-3.14t, side-path bug fixes with
   their regression tests, lint/type/hooks, CI, dormant release workflows, docs port, README, changelog,
