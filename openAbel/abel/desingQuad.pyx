...
#            temp0 = mathFun.sqrt(pl.grid[ii+1]**2-pl.grid[ii]**2)
...
#        for ii in range(pl.nData-1):
#            for jj in range(ii+2, pl.nData-1):
#                dataOut[ii] += dataInTemp1[jj] * pl.grid[jj]/mathFun.sqrt(pl.grid[jj]**2-pl.grid[ii]**2)
#            jj = pl.nData-1
#            dataOut[ii] += 0.5*dataInTemp1[jj] * pl.grid[jj]/mathFun.sqrt(pl.grid[jj]**2-pl.grid[ii]**2)
#            jj = ii+1
#            dataOut[ii] += 0.5*dataInTemp1[jj] * pl.grid[jj]/mathFun.sqrt(pl.grid[jj]**2-pl.grid[ii]**2)
#            jj = ii+1
#            dataOut[ii] += 0.5*(dataInTemp1[jj]-dataInTemp1[ii]) * pl.grid[jj]/mathFun.sqrt(pl.grid[jj]**2-pl.grid[ii]**2)
...
############################################################################################################################################
### Trapezoidal rule with linear desingularization                                                                                       ###



ctypedef struct methodData_DesingLin:
    double* desing
    double* coeffsDeriv
    int orderDeriv
    double* coeffsFilter
...
# Plan desingularized quadrature trapezoidal
cdef int plan_fat_trapezoidalDesingLin(abel_plan* pl) nogil except -1:

    cdef:
        methodData_DesingLin* md = <methodData_DesingLin*> malloc(sizeof(methodData_DesingLin))
        int ii, jj, ll
        double[::1] coeffs_filter_memView
        double temp0, temp1
        int orderFilterM1Half
    ...
    # Main method struct
    md = <methodData_DesingLin*> malloc(sizeof(methodData_DesingLin))
    if NULL == md:
        with gil:
            raise MemoryError('Malloc ruturned a NULL pointer, probably not enough memory available.')
    else:
    ...
    # Desingularize array
    md.desing = <double*> malloc((pl.nData-1)*2*sizeof(double))
    if NULL == md.desing:
    ...
    if pl.forwardBackward == -1:
        for ii in range(1,pl.nData-1):
            temp0 = mathFun.sqrt(pl.grid[pl.nData-1]**2-pl.grid[ii]**2)
            temp1 = mathFun.log((pl.grid[pl.nData-1]+temp0)/pl.grid[ii])
            md.desing[2*ii] = temp0/pl.stepSize
            md.desing[2*ii+1] = 0.5*((pl.grid[pl.nData-1]-2.*pl.grid[ii])*temp0 + pl.grid[ii]**2*temp1)/pl.stepSize
        temp0 = mathFun.sqrt(pl.grid[pl.nData-1]**2-pl.grid[0]**2)
        if pl.shift == 0.:
            md.desing[0] = temp0/pl.stepSize
            md.desing[1] = 0.
        elif pl.shift == 0.5:
            temp1 = mathFun.log((pl.grid[pl.nData-1]+temp0)/pl.grid[0])
            md.desing[0] = temp0/pl.stepSize
            md.desing[1] = 0.5*((pl.grid[pl.nData-1]-2.*pl.grid[0])*temp0 + pl.grid[0]**2*temp1)/pl.stepSize
        else:
            destroy_fat_trapezoidalDesingLin(pl)
            with gil:
                raise NotImplementedError('Method not implemented for given parameters.')
        ...
        for ii in range(1,pl.nData-1):
            temp0 = mathFun.sqrt(pl.grid[pl.nData-1]**2-pl.grid[ii]**2)
            temp1 = mathFun.log((pl.grid[pl.nData-1]+temp0)/pl.grid[ii])
            md.desing[2*ii] = temp1/pl.stepSize
            md.desing[2*ii+1] = (temp0 - pl.grid[ii]*temp1)/pl.stepSize
        temp0 = mathFun.sqrt(pl.grid[pl.nData-1]**2-pl.grid[0]**2)
        if pl.shift == 0.:
            md.desing[0] = 0.
            md.desing[1] = temp0/pl.stepSize
        elif pl.shift == 0.5:
            temp1 = mathFun.log((pl.grid[pl.nData-1]+temp0)/pl.grid[0])
            md.desing[0] = temp1/pl.stepSize
            md.desing[1] = (temp0 - pl.grid[0]*temp1)/pl.stepSize
    elif pl.forwardBackward == -2:
        for ii in range(pl.nData-1):
            temp0 = mathFun.sqrt(pl.grid[pl.nData-1]**2-pl.grid[ii]**2)
            md.desing[2*ii] = temp0/pl.grid[pl.nData-1]/pl.stepSize
            md.desing[2*ii+1] = pl.grid[ii]*(mathFun.acos(pl.grid[ii]/pl.grid[pl.nData-1])-temp0/pl.grid[pl.nData-1])/pl.stepSize
    else:
        destroy_fat_trapezoidalDesingLin(pl)
        with gil:
            raise NotImplementedError('Method not implemented for given parameters.')
    ...
    md.orderDeriv = 3
    md.coeffsDeriv = <double*> malloc(md.orderDeriv**2*sizeof(double))
    if NULL == md.coeffsDeriv:
        destroy_fat_trapezoidalDesingLin(pl)
        with gil:
            raise MemoryError('Malloc ruturned a NULL pointer, probably not enough memory available.')
    with gil:
        try:
            ...
            destroy_fat_trapezoidalDesingLin(pl)
            raise
    for ii in range(md.orderDeriv):
        md.coeffsDeriv[ii] = coeffs_filter_memView[ii]/pl.stepSize
...
# Execute desingularized quadrature trapezoidal
cdef int execute_fat_trapezoidalDesingLin(abel_plan* pl, double* dataIn, double* dataOut, int leftBoundary, int rightBoundary) nogil except -1:

    cdef:
        int ii, jj, nn
        methodData_DesingLin* md
        double* dataInTemp0 = NULL
        double* dataInTemp1 = NULL
        double* dataInDTemp = NULL
        int orderFilterM1Half, orderDerivM1Half
        int nLeftExt, nRightExt

    md = <methodData_DesingLin*> pl.methodData
    orderFilterM1Half = (md.orderFilter-1)/2
    orderDerivM1Half = (md.orderDeriv-1)/2

    # Allocate temporary data arrays
    dataInTemp0 = <double*> malloc((pl.nData+md.orderFilter+md.orderDeriv-2)*sizeof(double))
    dataInTemp1 = <double*> malloc((pl.nData+md.orderDeriv-1)*sizeof(double))
    dataInDTemp = <double*> malloc(pl.nData*sizeof(double))
    if NULL == dataInTemp0 or NULL == dataInTemp1 or NULL == dataInDTemp:
        free(dataInTemp0)
        free(dataInTemp1)
        free(dataInDTemp)
        with gil:
    ...
    # Left boundary handling
    if leftBoundary == 0 or leftBoundary == 1 or leftBoundary == 2:
        nLeftExt = orderFilterM1Half+orderDerivM1Half
    elif leftBoundary == 3:
        nLeftExt = 0
    else:
        with gil:
    ...
    # Right boundary handling
    if rightBoundary == 0: # TODO or rightBoundary == 1 or rightBoundary == 2:
        nRightExt = orderFilterM1Half+orderDerivM1Half
    elif rightBoundary == 3:
        nRightExt = 0
    else:
        free(dataInTemp0)
    ...
    # Copy and extend data if necessary
    nn = md.orderDeriv-1
    for ii in range(pl.nData+md.orderDeriv+md.orderFilter-2-nLeftExt-nRightExt):
        dataInTemp0[nLeftExt+ii] = dataIn[ii]
    if leftBoundary == 0:
        for ii in range(nLeftExt):
            dataInTemp0[ii] = polint(&dataInTemp0[nLeftExt], nn, ii-nLeftExt)
    ...
    if rightBoundary == 0:
        for ii in range(nRightExt):
            dataInTemp0[pl.nData+orderFilterM1Half+orderDerivM1Half+ii] = polint(&dataInTemp0[pl.nData+orderFilterM1Half+orderDerivM1Half-nn], nn, ii+nn)
    elif rightBoundary == 3:
        pass
    else:
        free(dataInTemp0)
    ...
    # Do scaling and/or numerical derivative
    convolve(dataInTemp0, pl.nData+md.orderDeriv-1, dataInTemp1, md.orderFilter, md.coeffsFilter)
    free(dataInTemp0)
    convolve(dataInTemp1, pl.nData, dataInDTemp, md.orderDeriv, md.coeffsDeriv)
    ...
    if pl.forwardBackward == -1:
        for ii in range(pl.nData-1):
            for jj in range(ii+1, pl.nData-1):
                dataOut[ii] += (dataInTemp1[orderDerivM1Half+jj]-dataInTemp1[orderDerivM1Half+ii]-dataInDTemp[ii]*(pl.grid[jj]-pl.grid[ii])) * \
                               pl.grid[jj]/mathFun.sqrt(pl.grid[jj]**2-pl.grid[ii]**2)
            dataOut[ii] += 0.5*(dataInTemp1[orderDerivM1Half+jj]-dataInTemp1[orderDerivM1Half+ii]-dataInDTemp[ii]*(pl.grid[jj]-pl.grid[ii])) * \
                               pl.grid[jj]/mathFun.sqrt(pl.grid[jj]**2-pl.grid[ii]**2)
            dataOut[ii] += dataInTemp1[orderDerivM1Half+ii]*md.desing[2*ii] + dataInDTemp[ii]*md.desing[2*ii+1]
    elif pl.forwardBackward == 1 or pl.forwardBackward == 2:
        for ii in range(pl.nData-1):
            for jj in range(ii+1, pl.nData-1):
                dataOut[ii] += (dataInTemp1[orderDerivM1Half+jj]-dataInTemp1[orderDerivM1Half+ii]-dataInDTemp[ii]*(pl.grid[jj]-pl.grid[ii])) * \
                               1./mathFun.sqrt(pl.grid[jj]**2-pl.grid[ii]**2)
            dataOut[ii] += 0.5*(dataInTemp1[orderDerivM1Half+jj]-dataInTemp1[orderDerivM1Half+ii]-dataInDTemp[ii]*(pl.grid[jj]-pl.grid[ii])) * \
                               1./mathFun.sqrt(pl.grid[jj]**2-pl.grid[ii]**2)
            dataOut[ii] += dataInTemp1[orderDerivM1Half+ii]*md.desing[2*ii] + dataInDTemp[ii]*md.desing[2*ii+1]
    elif pl.forwardBackward == -2:
        for ii in range(pl.nData-1):
            for jj in range(ii+1, pl.nData-1):
                dataOut[ii] += (dataInTemp1[orderDerivM1Half+jj]-dataInTemp1[orderDerivM1Half+ii]-dataInDTemp[ii]*(pl.grid[jj]-pl.grid[ii])) * \
                               (pl.grid[ii]/pl.grid[jj])**2/mathFun.sqrt(pl.grid[jj]**2-pl.grid[ii]**2)
            dataOut[ii] += 0.5*(dataInTemp1[orderDerivM1Half+pl.nData-1]-dataInTemp1[orderDerivM1Half+ii]-dataInDTemp[ii]*(pl.grid[pl.nData-1]-pl.grid[ii])) * \
                               (pl.grid[ii]/pl.grid[pl.nData-1])**2/mathFun.sqrt(pl.grid[pl.nData-1]**2-pl.grid[ii]**2)
            dataOut[ii] += dataInTemp1[orderDerivM1Half+ii]*md.desing[2*ii] + dataInDTemp[ii]*md.desing[2*ii+1]
    else:
        free(dataInTemp1)
        free(dataInDTemp)
...
cdef int destroy_fat_trapezoidalDesingLin(abel_plan* pl) nogil except -1:

    cdef:
        methodData_DesingLin* md = <methodData_DesingLin*> pl.methodData
